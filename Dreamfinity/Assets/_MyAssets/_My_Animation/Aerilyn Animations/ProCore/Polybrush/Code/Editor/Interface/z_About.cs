...
namespace Polybrush
{
	public class z_About : EditorWindow
	{
		...
		void OnGUI()
		{
			...
			if(GUILayout.Button(" GameTextures.com "))
				Application.OpenURL(z_Pref.GameTexturesLink);
				
			GUILayout.Space(12);
			
			EndHorizontalCenter();

			GUILayout.Label("<b>Notes</b>", EditorStyles.largeLabel);

			GUILayout.Label("Textures used in example scenes generously provided by GameTextures.com.", EditorStyles.wordWrappedLabel);
